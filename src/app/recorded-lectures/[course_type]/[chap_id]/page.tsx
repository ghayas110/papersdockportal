...
const AddLecture: React.FC<AddLectureProps> = ({ params }) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    if (selectedLecture) {
      try {
        const response = await fetch('http://be.papersdock.com/lectures/delete-lecture', {
          method: 'POST',
          headers: {
  ...
  const handleConfirmAdd = async () => {
    ...
    try {
      ...
      for (let chunkNumber = 1; chunkNumber <= totalChunks; chunkNumber++) {
        ...
        try {
          const response = await fetch('http://be.papersdock.com/lectures/create-lecture', {
            method: 'POST',
            headers: {
              accesstoken: `Bearer ${accessToken}`,
              'x-api-key': 'lms_API',
  ...
  const handleConfirmEdit = async () => {
    try {
      ...
      if (selectedLecture) {
        ...
        const response = await fetch('http://be.papersdock.com/lectures/update-lecture', {
          method: 'POST',
          headers: {
            accesstoken: `Bearer ${accessToken}`,
            'x-api-key': 'lms_API',
  ...
  const handleViewLecture = (lecture: Lecture) => {
    setVideoUrl(`http://be.papersdock.com${lecture.file_url}`);
