...
const ViewAssignments: React.FC<ViewAssignmentsProps> = ({ params }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const fetchedAssignments = filteredAssignments.map((assignment: any) => ({
          ...
          questionFile: `https://be.papersdock.com${assignment.assignment_file}`,
  ...
  const handleMarksSubmit = async (submissionId: string) => {
    ...
    if (marks) {
      try {
        const response = await fetch('https://be.papersdock.com/submission/check-assignment', {
          method: 'POST',
          headers: {
            'accesstoken': `Bearer ${accessToken}`,
            'x-api-key': 'lms_API',
  ...
  const handleViewAssignment = (fileUrl: string) => {
    setPdfUrl(`https://be.papersdock.com${fileUrl}`);
    setIsModalOpen(true);
  };
