...
const StudentFeePage: React.FC = () => {
  ...
  const fetchFeeData = async () => {
    try {
      const response = await fetch('https://be.papersdock.com/fees/get-all-fee-august-to-may', {
        headers: {
  ...
  const handleOk = async () => {
    if (selectedFee && fileList.length > 0) {
      try {
        ...
        const response = await fetch('https://be.papersdock.com/fees/insert-fee-invoice', {
          method: 'POST',
          headers: {
            'accesstoken': `Bearer ${accessToken}`,
            'x-api-key': 'lms_API',
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string, record: FeeData) =>
        record.invoice_file ? (
          <a href={`https://be.papersdock.com${record.invoice_file}`} target="_blank" rel="noopener noreferrer">
            View Invoice
          </a>
        ) : (
          'No Invoice'
          ...
          {selectedFee && (
            <>
              ...
              {selectedFee.invoice_file ? (
                <p>
                  Existing Invoice:{' '}
                  <a href={`https://be.papersdock.com${selectedFee.invoice_file}`} target="_blank" rel="noopener noreferrer">
                    View Invoice
                  </a>
                </p>
              ) : (
