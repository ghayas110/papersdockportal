...
const AddNotes: React.FC<AddNotesProps> = ({ params }) => {
  ...
  const handleEditNote = (note: Note) => {
    ...
    setBgImageFileList([{
      ...
      status: 'done',
      url: `https://www.be.papersdock.com${note.note_bg_image}`,
    }]);
    ...
    setNotesFileList([{
      ...
      status: 'done',
      url: `https://www.be.papersdock.com${note.note_type === 'dark_mode' ? note.dark_note_attachment : note.light_note_attachment}`,
    }]);
  ...
  const handleConfirmDelete = async () => {
    ...
    if (selectedNote) {
      try {
        const response = await fetch('https://www.be.papersdock.com/notes/delete-notes', {
          method: 'POST',
          headers: {
  ...
  const handleConfirmAdd = async () => {
    ...
    try {
      ...
      const response = await fetch('https://www.be.papersdock.com/notes/create-notes', {
        method: 'POST',
        headers: {
          'accesstoken': `Bearer ${accessToken}`,
          'x-api-key': 'lms_API',
  ...
  const handleConfirmEdit = async () => {
    ...
    try {
      ...
      if (selectedNote) {
        ...
        const response = await fetch('https://www.be.papersdock.com/notes/update-notes', {
          method: 'POST',
          headers: {
            'accesstoken': `Bearer ${accessToken}`,
            'x-api-key': 'lms_API',
  ...
  const handleViewImage = (note: Note) => {
    setImageUrl(`https://www.be.papersdock.com${note.note_bg_image}`);
